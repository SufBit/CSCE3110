...
void routeSearch_2(Graph graph, int city_A, int city_B, int city_C, int city_D) {
    ...
    // Find the shortest path from city B to city C
    ...
    // Find the shortest path from city C to city D
    Dijkstra(graph, city_C, d1, p1);
    if(d1[city_D] == INT_MAX){
        cout << "No route." << endl;
        return;
    }

	//cout << "This is a check" << p1[city_D] << d1[city_D];

	int connections = d1[city_B] + d1[city_C] + d1[city_D] + 1;

    // Combine the three shortest paths to form the final route
    cout<< endl;
    cout << city_A << " to ";
    i = p1[city_B];
    while(i != -1){
    ...
    cout << city_D << endl;
	cout << "Total connection: " << connections << endl;
}
			

//To print path for routeSearch3
...
void routeSearch_3(Graph graph, int city_A, int city_B, int city_C) {
	...
	//Arrays to store distances and previous nodes
	int dist_A[graph.get()], dist_B[graph.get()], dist_C[graph.get()];
    int prev_A[graph.get()], prev_B[graph.get()], prev_C[graph.get()];

	//To find shortest paths from each person
    Dijkstra(graph, city_A, dist_A, prev_A);
    Dijkstra(graph, city_B, dist_B, prev_B);
    Dijkstra(graph, city_C, dist_C, prev_C);

	//Minimize total connections
    ...
    for (int i = 0; i < graph.get(); i++) {
        ...
        int total_connections = dist_A[i] + dist_B[i] + dist_C[i];

		// Update the minimum if this city requires fewer connections
        if (total_connections < min_total) {
            min_total = total_connections;
            best_city = i;
        }
    }

    if (best_city == -1) {
        cout << "There is no city that minimizes the total number of connections." << endl;
    } else {
        cout << "The city that minimizes the total number of connections is " << best_city << "." << endl;
        cout << "The total number of connections is " << min_total << "." << endl;

        //Paths from each person's starting city to the meeting city
        cout << "Route from city " << city_A << " to city " << best_city << ":" << endl;
        printPath(prev_A, city_A, best_city);
        cout << endl;

        cout << "Route from city " << city_B << " to city " << best_city << ":" << endl;
        printPath(prev_B, city_B, best_city);
        cout << endl;

        cout << "Route from city " << city_C << " to city " << best_city << ":" << endl;
        printPath(prev_C, city_C, best_city);
        cout << endl;
    }
